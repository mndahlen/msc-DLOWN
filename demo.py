...
def Fig_3_5_a_E21_1():
    ...
    # plot
    hfont1= {'fontname':FONTNAME,'fontsize':14,'fontstyle':'normal'}
    hfont2= {'fontname':FONTNAME,'fontsize':18,'fontstyle':'normal'}
    ...
    # plt.title("Expected throughput " + r"$\mathbb{E}[Q]$" + " in erdos topology with 20 devices\nfor various S", **hfont1)
...
def E12_average_and_expected_num_successful_links_ring():
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E13_average_and_expected_num_successful_links_complete():
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def graph_plot_schedule_collision():
    torch.manual_seed(1)
    random.seed(3)
    np.random.seed(1)
...
def graph_plot_schedule_interference_free():
    torch.manual_seed(1)
    random.seed(3)
    np.random.seed(1)
...
def plot_graph():
    ...
    hfont1= {'fontname':FONTNAME,'fontsize':14,'fontstyle':'normal'}
    hfont2= {'fontname':FONTNAME,'fontsize':12,'fontstyle':'normal'}
    plt.show()

def explore_clusters_dataset():
    random.seed(3)
    np.random.seed(7)
    num_users = 20
    iid = True
...
def E2_ring_graph_IF_model_line_line_random_policy():
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E3_ring_graph_IF_model_linearmodel_4clusters_random_policy():
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E4_1_erdos_graph_CD_model_line_line_random_policy():
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E5_1_erdos_graph_CD_model_linear_model_4clusters_random_policy():
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E9_1_IID_IF_line_line_random_policy():
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E14_1_multiple_trials_random_policy_and_optimal(save=False):
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
    ...
    # calculate optimal probability wrt expected num links and also min spectral radius
    probs = np.arange(0,1,0.001)
    num_expected_successful = np.zeros((len(probs)))
    for i in range(len(probs)):
...
def E18_deterministic_optimal_vs_random_optimal():
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E49_1_random_policy_delay_uniform_equal_throughput_different_slots(save=False):
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E33_random_policy_delay_uniform(save=False):
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E34_random_policy_delay_uniform_over_slots(save=False):
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
    ...
    fig, ax = plt.subplots(1)
    plt.plot([t for t in times], losses[0], "-", color="black", linewidth=1)#, label="{}: {}".format(labels[0], policy.get_optimal_prob_throughput_S(1)), linewidth=1)
    plt.plot([3*t for t in times], losses[1], "-", color="black", linewidth=1)#, label="{}: {}".format(labels[1], policy.get_optimal_prob_throughput_S(3)), linewidth=1)
    plt.plot([6*t for t in times], losses[2], "-", color="black", linewidth=1)#, label="{}: {}".format(labels[2], policy.get_optimal_prob_throughput_S(6)), linewidth=1)
    plt.plot([20*t for t in times], losses[3], "-", color="black", linewidth=1)#, label="{}: {}".format(labels[3], policy.get_optimal_prob_throughput_S(20)), linewidth=1)
    plt.plot([50*t for t in times], losses[4], "-", color="black", linewidth=1)#, label="{}: {}".format(labels[4], policy.get_optimal_prob_throughput_S(50)), linewidth=1)
...
def E22_random_policy_delay_uniform(save=False):
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E38_random_policy_delay_uniform_clusters(save=False):
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E23_random_policy_delay_uniform(save=False):
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
    ...
    data.train.plot()
    data.test.plot()
    K_uniform = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a=1, b=40)
    labels={0:"S=1", 1:"S=3", 2:"S=6", 3:"S=20", 4:"S=50", 5:"S_t=10"}
    K_1 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 3, b=40)   
    K_2 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 6, b=40)   
    K_3 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 20, b=40)   
    K_4 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 50, b=40)   
    plt.plot(K_uniform, label=labels[0])
    plt.plot(K_1, label=labels[1])
    plt.plot(K_2, label=labels[2])
    plt.plot(K_3, label=labels[3])
    ...
    # optional save
    if 0:
        np.save("experimentdata/E23.npy",np.array([losses]))
    # plotting
    plt.plot(times[0], losses[0], "-*", label="{}: {}".format(labels[0], policy.get_optimal_prob_throughput_S(1)))
    plt.plot(times[1][0:np.argmin(losses[1])], losses[1][0:np.argmin(losses[1])], "-.", label="{}: {}".format(labels[1], policy.get_optimal_prob_throughput_S(3)))
    plt.plot(times[2][0:np.argmin(losses[2])], losses[2][0:np.argmin(losses[2])], "--", label="{}: {}".format(labels[2], policy.get_optimal_prob_throughput_S(6)))
    ...
    plt.ylabel("Loss")
    plt.legend()
    name = "E23 delay uniform"
    plt.title(name)
    plt.grid(True)
    plt.show()

def E24_random_policy_delay_uniform(save=False):
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
    ...
    data.train.plot()
    data.test.plot()
    K_uniform = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a=1, b=80)
    labels={1:"S_t=3", 2:"S_t=4", 3:"S_t=5", 4:"S_t=6", 5:"S_t=10"}
    K_1 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 3, b=80)   
    K_2 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 6, b=80)   
    K_3 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 20, b=80)   
    K_4 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 50, b=80)   
    plt.plot(K_uniform, label="uniform")
    plt.plot(K_1, label=labels[1])
    plt.plot(K_2, label=labels[2])
    plt.plot(K_3, label=labels[3])
    plt.plot(K_4, label=labels[4])
    ...
    for trial in range(num_trials):
        print("Trial #{}, {}".format(trial,labels[2]))
        policy = Policy(graph, policy="random", p=policy.get_optimal_prob_throughput_S(4))
        result = dfl(graph.W, data.test, devices.devices, policy, comm, args, S=K_2)
        performance = result["performance"]
        energies = result["energies"]
        times.append([p[0] for p in performance])
    ...
    for trial in range(num_trials):
        print("Trial #{}, {}".format(trial, labels[3]))
        policy = Policy(graph, policy="random", p=policy.get_optimal_prob_throughput_S(5))
        result = dfl(graph.W, data.test, devices.devices, policy, comm, args, S=K_3)
        performance = result["performance"]
        energies = result["energies"]
        times.append([p[0] for p in performance])
    ...
    # optional save
    if 0:
        np.save("experimentdata/E24.npy",np.array([losses]))
    # plotting
    plt.plot(times[0], losses[0], "-*", label="uniform: {}".format(policy.get_optimal_prob_throughput_S(1)))
    plt.plot(times[1][0:np.argmin(losses[1])], losses[1][0:np.argmin(losses[1])], "-.", label="{}: {}".format(labels[1], policy.get_optimal_prob_throughput_S(3)))
    plt.plot(times[2][0:np.argmin(losses[2])], losses[2][0:np.argmin(losses[2])], "--", label="{}: {}".format(labels[2], policy.get_optimal_prob_throughput_S(4)))
    plt.plot(times[3][0:np.argmin(losses[3])], losses[3][0:np.argmin(losses[3])], "^-", label="{}: {}".format(labels[3], policy.get_optimal_prob_throughput_S(5)))
    plt.plot(times[4][0:np.argmin(losses[4])], losses[4][0:np.argmin(losses[4])], ".", label="{}: {}".format(labels[4], policy.get_optimal_prob_throughput_S(6)))
    plt.xlabel("Iteration")
    plt.ylabel("Loss")
    plt.legend()
    name = "E24 delay uniform, delay=80, B=300"
    plt.title(name)
    plt.grid(True)
    plt.show()
...
def E40_random_policy_delay_uniform_clusters(save=False):
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
    ...
    data.train.plot()
    data.test.plot()
    K_uniform = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a=1, b=500)
    labels={0:"S_t=1", 1:"S_t=3", 2:"S_t=6", 3:"S_t=20", 4:"S_t=50", 5:"S_t=10"}
    K_1 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 3, b=500)   
    K_2 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 6, b=500)   
    K_3 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 20, b=500)   
    K_4 = resource.get(T=args.T, type="delay_uniform", per_round=1, constraint=constraint, a = 50, b=500)   
    plt.plot(K_uniform, label=labels[0])
    plt.plot(K_1, label=labels[1])
    plt.plot(K_2, label=labels[2])
    plt.plot(K_3, label=labels[3])
    ...
    # optional save
    if 0:
        np.save("experimentdata/E40.npy",np.array([accuracies, losses]))
    # plotting
    plt.plot(times[0], losses[0], "-*", label="{}: {}".format(labels[0], policy.get_optimal_prob_throughput_S(1)))
    plt.plot(times[1][0:np.argmin(losses[1])], losses[1][0:np.argmin(losses[1])], "-.", label="{}: {}".format(labels[1], policy.get_optimal_prob_throughput_S(3)))
    plt.plot(times[2][0:np.argmin(losses[2])], losses[2][0:np.argmin(losses[2])], "--", label="{}: {}".format(labels[2], policy.get_optimal_prob_throughput_S(6)))
    ...
    plt.ylabel("Loss")
    plt.legend()
    name = "E40 delay uniform"
    plt.title(name)
    plt.grid(True)
    plt.show()

def E52_2_periodic_allocation_line(save=False):
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
...
def E53_2_periodic_allocation_cluster(save=False):
    args = args_parser()
    args.device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() and args.gpu != -1 else 'cpu')
    torch.manual_seed(1)
    random.seed(4)
